...
3. **Aspect-Level Binary Polarity**: Dedicated binary classification per aspect (Positive vs. Negative) using class-balanced TF-IDF Logistic Regression.
4. **Dual Model Benchmarking**: Compares TF-IDF FeatureUnion (Word + Char n-grams) with a custom PyTorch BiLSTM network for Sentiment and Aspect detection.
5. **Preprocessing**: Normalizes Unicode NFC, strips HTML/URLs/emails/elongations/noise, and preserves sentiment-critical negations (e.g., না, নাই, নয়, নেই).
...
├── project_report/                  # Academic laboratory report (LaTeX & diagrams)
│   ├── figures/                     # Architecture & pipeline diagrams
│   ├── compile.sh                   # Compilation shell script
│   ├── generate_diagrams.py         # Matplotlib diagram generator
│   ├── main.tex                     # LaTeX report source document
